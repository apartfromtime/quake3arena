...
static int (Q_CDECL *syscall)( int arg, ... ) = (int (Q_CDECL *)( int, ...))-1;

void dllEntry( int (Q_CDECL *syscallptr)( int arg,... ) ) {
	syscall = syscallptr;
}
...
qhandle_t	trap_S_RegisterSound( const char *sample, bool compressed ) {
