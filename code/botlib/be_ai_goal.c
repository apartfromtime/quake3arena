...
bot_goalstate_t *BotGoalStateFromHandle(int handle)
{
	...
	{
		g_bimport.Print(PRT_FATAL, "goal state handle %d out of range\n", handle);
		return NULL;
	} //end if
	if (!botgoalstates[handle])
	{
		g_bimport.Print(PRT_FATAL, "invalid goal state %d\n", handle);
		return NULL;
	} //end if
...
itemconfig_t *LoadItemConfig(char *filename)
{
	...
	if (max_iteminfo < 0)
	{
		g_bimport.Print(PRT_ERROR, "max_iteminfo = %d\n", max_iteminfo);
		max_iteminfo = 256;
	...
	FreeSource(source);
	//
	if (!ic->numiteminfo) g_bimport.Print(PRT_WARNING, "no item info loaded\n");
...
levelitem_t *AllocLevelItem(void)
{
	...
	if (!li)
	{
		g_bimport.Print(PRT_FATAL, "out of level items\n");
		return NULL;
	} //end if
	//
...
void BotInitInfoEntities(void)
{
	...
	for (ent = AAS_NextBSPEntity(0); ent; ent = AAS_NextBSPEntity(ent))
	{
		...
		else if (!strcmp(classname, "info_camp"))
		{
			...
			if (!cs->areanum)
			{
				g_bimport.Print(PRT_MESSAGE, "camp spot at %1.1f %1.1f %1.1f in solid\n", cs->origin[0], cs->origin[1], cs->origin[2]);
				FreeZoneMemory(cs);
				continue;
			} //end if
			cs->next = campspots;
	...
	if (bot_developer)
	{
		g_bimport.Print(PRT_MESSAGE, "%d map locations\n", numlocations);
		g_bimport.Print(PRT_MESSAGE, "%d camp spots\n", numcampspots);
	} //end if
...
void BotInitLevelItems(void)
{
	...
	for (ent = AAS_NextBSPEntity(0); ent; ent = AAS_NextBSPEntity(ent))
	{
		...
		{
			g_bimport.Print(PRT_ERROR, "item %s without origin\n", classname);
			continue;
		} //end else
		//
		goalareanum = 0;
		...
		else
		{
			...
			if (!li->goalareanum)
			{
				g_bimport.Print(PRT_MESSAGE, "%s not reachable for bots at (%1.1f %1.1f %1.1f)\n",
												classname, origin[0], origin[1], origin[2]);
			} //end if
		} //end else
		//
	...
	g_bimport.Print(PRT_MESSAGE, "found %d level items\n", numlevelitems);
...
void BotPushGoal(int goalstate, bot_goal_t *goal)
{
	...
	{
		g_bimport.Print(PRT_ERROR, "goal heap overflow\n");
...
int BotLoadItemWeights(int goalstate, char *filename)
{
	...
	if (!gs->itemweightconfig)
	{
		g_bimport.Print(PRT_FATAL, "couldn't load weights\n");
...
void BotFreeGoalState(int handle)
{
	...
	if (!botgoalstates[handle])
	{
		g_bimport.Print(PRT_FATAL, "invalid goal state handle %d\n", handle);
		return;
	} //end if
...
int BotSetupGoalAI(void)
{
	...
	if (!itemconfig)
	{
		g_bimport.Print(PRT_FATAL, "couldn't load item config\n");
