...
===================
*/
void Com_ParseError(const char* msg, ...)
{
	va_list		argptr;
	char		string[32000];
...
void Com_ParseWarning(const char* msg, ...)
{
	va_list		argptr;
	char		string[32000];
...
char* Com_ParseExt(const char* (*data_p), bool allowLineBreaks)
{
	...
	// we still allow star, dash, pound, forward and back slashes in name tokens for pathnames
...
void Com_MatchToken(const char* (*buf_p), const char* match, bool warning)
{
	...
	if (strcmp(token, match)) {
		if (warning) {
			Com_ParseWarning("MatchToken: %s != %s", token, match);
		}
		else {
