...
static void LoadJPG( const char *filename, byte **pic, int *width, int *height ) {
  ...
  fbuffersize = ri.FS_ReadFile ( ( char * ) filename, (void **)&fbuffer);
  if (!fbuffer) {
	return;
  }
...
qhandle_t RE_RegisterSkin( const char *name ) {
	...
	if ( strcmp( name + strlen( name ) - 5, ".skin" ) ) {
		skin->numSurfaces = 1;
		skin->surfaces[0] = ri.Hunk_Alloc( sizeof( skinSurface_t ) );
	...
	while ( text_p && *text_p ) {
		...
		surf = skin->surfaces[ skin->numSurfaces ] = ri.Hunk_Alloc( sizeof( skinSurface_t ) );
